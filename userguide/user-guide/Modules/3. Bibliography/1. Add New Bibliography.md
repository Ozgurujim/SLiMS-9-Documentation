...
6. Item (s) Code Batch Generator. In this field, we are required to select the template code pattern first. If you don't have an exemplary pattern, click on the "Add New Pattern" button to add an exemplary pattern.
   ...
   ![img](https://lh3.googleusercontent.com/d4Zy8nuQCVxpE0U5bZYbBPFA5wByzdb09MwXnTN4RBzs6uNyVJTT77hQUuY4CONd01MMVnO04KNCoC0tQ7HYYtorFicrS-BPTAsWbdeZFUIJ2Vo1jwTUYvQO1dFH1SjnK7XmuX60) However at this stage the Call Number field will not appear, as that data hasn't been entered yet in the main bibliographic record.
...
15. Classification. Classification number by subject (DDC, UDC or other). Begin typing and a drop-down selector will offer Classification numbers previously used. If the number you want is in the list, select it. If not in the list, keep typing your number until <Addnew> appears alongside your entry, and select that.

16. Call Number. Item Call Numbers. Attached to the spine, is representative of the subject when placed on a shelf and determines shelving order. When you have already entered a Classification number, it will prefill the Call Number field, which can then be edited to add a prefix and/or suffix. If you have previously used the Item Code Batch Generator, when you complete this Call Number Field this will be allocated to all items, although individual items can then be edited to account for possible locations etc.  If you have not created any items, and return to create them, the Call Number field will now be pre-filled.

17. Subjects. Topic / subject. This data can also be retrieved from the Master File by first filling in the subject to be entered. By entering the subject first, one of the advantages is that a classification number can be assigned to each subject, for a preliminary classification.

18. Language. Choice of language that the processed library materials are written in.
