...
- *User type* [Senior Librarian/Librarian/Library staff]
...
![library_system_users1](library_system_users1.png)

Within user information, a user can also add their internet credentials, e.g. Facebook profile, blog or websites, etc.

To add a new user, click **Add New User,** and then fill in the Login username, Real name, User type ( that they will belong to), and Password. If a password policy has been set, the password must conform to it. These fields are mandatory.
...
Checkboxes permit optional allocation to security Group(s): e.g Admin , Circulation, Librarians, Visitors etc that have been previously created via the User Group option, This allows the system administrator to determine which menu items are available to the logged-in library staff.

![library_system_users2](library_system_users2.png)
